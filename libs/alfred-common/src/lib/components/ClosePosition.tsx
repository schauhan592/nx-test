...
interface Props {
  payload: VaultClosePositionsPayload | undefined;
...
const steps = [
  {
    ...
    description: `Creating Transaction to closing the position.`,
  },
  {
    label: 'Sign Transaction',
    description: 'Open your wallet and sign Transaction to close the position.',
  },
  {
    label: 'Position closed successfully',
...
export default function ClosePositionTxn(props: Props) {
  const { payload, setStatus, status } = props;
  const [activeStep, setActiveStep] = useState<number>(0);
  const { enqueueSnackbar } = useSnackbar();
  const [initiated, setInitiated] = useState<boolean>(false);

  const { mutate, data, error } = useClosePosition();
  ...
  useEffect(() => {
    if (!isTxnInitiated && txnStatus && txnStatus?.status === 'txConfirmed') {
      setStatus({
        ...status,
        isLoading: false,
        success: { isSuccess: true, message: 'You have successfully closed the position' },
      });
      setActiveStep(3);
  ...
  useEffect(() => {
    if (data) {
      if (data?.isSkip && Boolean(data?.isSkip) === true) {
        setStatus({
          ...status,
          isLoading: false,
          success: { isSuccess: true, message: 'Position is already closed' },
        });
        setActiveStep(3);
        props.setActiveStep(1);
      } else {
  ...
  async function signRawTxn(txn: TXN_TYPE) {
    const rawTx = {
      ...txn,
      gasLimit: BigNumber.from(txn?.gasLimit),
      maxPriorityFeePerGas: BigNumber.from(txn?.maxPriorityFeePerGas),
    };
