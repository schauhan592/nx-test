...
import { CorrelationDataPoints } from '@alfred/alfred-common';
...
class D3CorrelationChart {
  ...
  constructor(containerEl: any, props: D3CorrelationChartProps) {
    ...
    this.mostActivePrice = this.renderMostActivePriceAssumption(props.mostActivePrice);
  ...
  handleMouseMove() {
    ...
    const onMouseMove = (e: any) => {
      ...
      focus.attr('cx', this.x(selectedData.x)).attr('cy', this.y(selectedData.y));
      ...
      if (this.x(selectedData.x) > this.props.width * 0.8) {
        focusText
          .html(
            'Price: ' +
              selectedData.y.toFixed(4) +
              '<br>' +
              ' Date:' +
              new Date(selectedData.x).toLocaleDateString()
          )
          .attr('x', function () {
  ...
  updateMinMaxPriceRange(minPrice: number, maxPrice: number, isFullRange: boolean) {
